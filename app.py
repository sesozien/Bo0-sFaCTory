...
def save_styled_excel(file_path, new_row):
        ...
        try:
            ...
            for _, r in old_df.iterrows():
                if pd.notna(r.iloc[0]) and not str(r.iloc[0]).startswith("🛸") and not str(r.iloc[0]).startswith("كود"):
                    # نقرأ أول 7 أعمدة فقط لضمان النظافة
                    rows_to_write.append(dict(zip(columns_keys, list(r[:7]))))
        except:
            pass

    # إضافة المنتج الجديد القادم من البوت
    rows_to_write.append(new_row)
    df_all = pd.DataFrame(rows_to_write)

    # بدء الكتابة والتنسيق الاحترافي
    ...
    # تفعيل خيار العرض من اليمين لليسار (RTL) لفخامة اللغة العربية
    worksheet.right_to_left()
    
    # باليتة الألوان الهادئة (Pastel Muted)
    ...
    for row_idx in range(len(df_all)):
        ...
        for col_idx in range(7):
            ...
            # تحديد التنسيق بناءً على نوع البيانات (نصوص يمين، أرقام في النص)
            if col_idx in [1, 4]: # العنوان والوصف
    ...
    # إمضاء MR Bo0 الفخم يتحرك ديناميكياً في نهاية الملف
    ...
    # ضبط ارتفاع الصفوف وعرض الأعمدة تلقائياً
...
st.markdown("<br><p style='text-align: center; color: #2a4d69; font-weight: bold;'>🛸 تم التطوير والأداء بكل فخامة بواسطة: MR Bo0 🛸</p>", unsafe_allow_html=True)    Image.new('RGBA', (200, 200), color=(255, 75, 75, 255)).save(DEFAULT_LOGO_PATH)
if not os.path.exists(ACTIVE_LOGO_PATH):
    Image.open(DEFAULT_LOGO_PATH).save(ACTIVE_LOGO_PATH)
